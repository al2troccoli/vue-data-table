<template>
  <div class="al2-assetsForm ">
    ...
    <!--NOTE: Status-->
    <div class="al2-assetsForm__status" v-if="assetsFiltered[currentSelectedAssetIndex] !== undefined" @click="toggleState">
      <div v-if="assetsFiltered[currentSelectedAssetIndex].status" class="al2-assetsForm__status--enabled">Enabled</div>
      <div v-else="" class="al2-assetsForm__status--disabled">Disabled</div>
    </div>

    <form v-on:submit.prevent="" class="al2-assetsForm__form" v-if="assetsFiltered[currentSelectedAssetIndex] !== undefined">
      <!-- NOTE: Term field-->
      <div class="al2-assetsForm__form-group">
        <label class="al2-assetsForm__label" for="term">Term</label>
        <input class="al2-assetsForm__textinput" type="text" v-model="assetsFiltered[currentSelectedAssetIndex].term" id="term"/>
      </div>

      <!-- NOTE: Serial number -->
      <div class="al2-assetsForm__form-group">
        <label class="al2-assetsForm__label" for="serial_number">Serial number</label>
        <input class="al2-assetsForm__textinput" type="text" v-model="assetsFiltered[currentSelectedAssetIndex].serial_number" id="serial_number"/>
      </div>

      <!-- NOTE: Description -->
      <div class="al2-assetsForm__form-group">
        <label class="al2-assetsForm__label" for="description">Description</label>
        <textarea  class="al2-assetsForm__textarea" type="text" v-model="assetsFiltered[currentSelectedAssetIndex].description" id="description"/>
      </div>

      <!-- NOTE: Organisation -->
      <div class="al2-assetsForm__form-group">
        <label class="al2-assetsForm__label" for="organisation">Organisation</label>
        <input class="al2-assetsForm__textinput" type="text" v-model="assetsFiltered[currentSelectedAssetIndex].organization" id="organisation"/>
      </div>

      <!-- NOTE: Price -->
      <div class="al2-assetsForm__form-group">
        <label class="al2-assetsForm__label" for="price">Price in {{getCurrency()}}</label>
        <input class="al2-assetsForm__textinput" type="number" v-model="assetsFiltered[currentSelectedAssetIndex].price" id="price"/>
      </div>
